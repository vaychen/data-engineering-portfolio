-- =============================================================================
-- DDL: ods_nova_app_events
-- Layer   : ODS (Operational Data Store)
-- Source  : nova mobile application event stream (flatten_nova_record → S3 →
--           Redshift Spectrum → staging → dedupe → this table)
-- Refresh : Append-only; partitioned by partition_date
--
...
--                   event_timestamp (epoch ms BIGINT)
...
--                   (from nova_device_os / nova_device_os_version on
--                    nova_session_start rows; NULL for all other event types)
--   ProductContext: product_name, product_id, product_variant, product_guid,
--                   product_firmware_version
--                   (NULL on login and session events)
--   Session       : session_id  (← client_session_id)
--   Payload       : payload SUPER — event-specific nova_* fields
--   Partition     : partition_date  (← dt cast to DATE)
--
-- Distribution strategy:
--   DISTKEY  partition_date  — co-locates data processed together each day,
--                              making range-scan joins on date efficient.
--   SORTKEY  (partition_date, event_name) — supports filtered scans by
--                              event type within a date range without a
--                              full-table scan.
-- =============================================================================

CREATE TABLE IF NOT EXISTS analytics_dw.ods_nova_app_events
(
    -- -------------------------------------------------------------------------
    -- Event identity
    -- -------------------------------------------------------------------------
    ...
    -- -------------------------------------------------------------------------
    -- Partition column (also used as DISTKEY)
    -- -------------------------------------------------------------------------
    partition_date          DATE                NOT NULL    -- YYYY-MM-DD; matches Firehose dt partition
)
DISTKEY  (partition_date)
SORTKEY  (partition_date, event_name);

COMMENT ON TABLE analytics_dw.ods_nova_app_events IS
    'Raw nova mobile app events. Append-only. One row per event_guid. '
    'Partitioned and distributed on partition_date.';
