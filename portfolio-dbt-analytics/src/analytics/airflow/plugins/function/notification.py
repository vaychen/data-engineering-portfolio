"""
plugins/function/mail_notification.py

Airflow callback functions that publish pipeline health events to AWS SNS.
Attach `on_failure_callback` to `default_args` so every task failure triggers an alert.
Attach `on_success_callback` to the final task of a DAG for end-to-end success notification.
"""
